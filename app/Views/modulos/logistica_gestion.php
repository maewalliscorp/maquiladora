...
<div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <div>
        <button type="button" class="btn btn-outline-secondary me-2" data-bs-toggle="modal"
                data-bs-target="#transportistasModal">
            <i class="bi bi-truck me-1"></i> Gestionar Transportistas
        </button>
        <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#agregarModal">
            <i class="bi bi-plus-circle me-1"></i> Agregar Envío
        </button>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <dl class="row mb-0">
                    ...
                    <dt class="col-4">Transportista</dt>
                    <dd class="col-8" id="v-transportista">-</dd>
                    <dt class="col-4">Núm. guía</dt>
                    <dd class="col-8" id="v-guia">-</dd>
                    <dt class="col-4">Fecha salida</dt>
...
<!-- ===== MODAL: Gestionar Transportistas ===== -->
<div class="modal fade" id="transportistasModal" tabindex="-1" aria-labelledby="transportistasModalLabel"
     aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="transportistasModalLabel">Gestionar Transportistas</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <button type="button" class="btn btn-sm btn-primary mb-3" id="btnNuevoTransportista">
                    <i class="bi bi-plus-circle me-1"></i> Nuevo Transportista
                </button>
                <table class="table table-sm table-striped" id="tablaTransportistas">
                    <thead>
                    <tr>
                        <th>Nombre</th>
                        <th>Contacto</th>
...
<div class="modal fade" id="formTransportistaModal" tabindex="-1" aria-labelledby="formTransportistaModalLabel"
     aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="formTransportistaModalLabel">Nuevo Transportista</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="formTransportista">
                <div class="modal-body">
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text" class="form-control" id="t-contacto">
                    </div>
                    <div class="mb-2">
                        <label class="form-label">Teléfono</label>
                        <input type="text" class="form-control" id="t-telefono">
                    </div>
                    <div class="mb-2">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <div class="btn-group" role="group">
                            <button class="btn btn-outline-info btn-sm btn-icon btn-ver"
                                    data-id="<?= (int) $r['id'] ?>" title="Ver">
                                <i class="bi bi-eye"></i>
                            </button>
                            <button class="btn btn-outline-primary btn-sm btn-icon btn-editar"
                            ...
                            <?php if (!empty($r['urlSeguimiento'])): ?>
                                <a class="btn btn-outline-secondary btn-sm btn-icon"
                                   href="<?= esc($r['urlSeguimiento']) ?>"
                                   target="_blank" title="Abrir tracking">
                                    <i class="bi bi-box-arrow-up-right"></i>
                                </a>
                            <?php endif; ?>

                            <!-- Eliminar envío con SweetAlert2 -->
                            <form method="post"
                                  action="<?= site_url('modulo3/envios/' . (int) $r['id'] . '/eliminar') ?>"
                                  class="d-inline ms-1 form-eliminar-envio">
                                <?= csrf_field() ?>
...
<script>
    (function () {
        ...
        const baseTransportista = "<?= site_url('modulo3/transportistas') ?>";

        // ===== TRANSPORTISTAS =====
        async function cargarTransportistas() {
            const res = await fetch(`${baseTransportista}/listar`);
            const json = await res.json();
            const tbody = document.querySelector('#tablaTransportistas tbody');
            tbody.innerHTML = '';
            ...
            json.data.forEach(t => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${t.email || ''}</td>
                    <td>
                        <button class="btn btn-sm btn-outline-primary btn-edit-transportista" data-id="${t.id}">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn btn-sm btn-outline-danger btn-del-transportista" data-id="${t.id}">
                            <i class="bi bi-trash"></i>
                        </button>
                    </td>
                `;
            ...
            actualizarDropdownTransportistas(json.data);
        }

        function actualizarDropdownTransportistas(data) {
            const selects = document.querySelectorAll('select[name="transportistaId"]');
            selects.forEach(select => {
                const valorActual = select.value;
                const opciones = data.map(t =>
                    `<option value="${t.id}" ${t.id == valorActual ? 'selected' : ''}>${t.nombre}</option>`
                ).join('');
        ...
        // Nuevo transportista
        document.getElementById('btnNuevoTransportista').addEventListener('click', () => {
            document.getElementById('formTransportistaModalLabel').textContent = 'Nuevo Transportista';
            document.getElementById('t-id').value = '';
            document.getElementById('formTransportista').reset();
            new bootstrap.Modal(document.getElementById('formTransportistaModal')).show();
        });

        // Guardar transportista
        document.getElementById('formTransportista').addEventListener('submit', async (e) => {
            e.preventDefault();
            const id = document.getElementById('t-id').value;
            const data = new FormData();
            data.append('nombre', document.getElementById('t-nombre').value);
            data.append('contacto', document.getElementById('t-contacto').value);
            data.append('telefono', document.getElementById('t-telefono').value);
            data.append('email', document.getElementById('t-email').value);

            const url = id ? `${baseTransportista}/${id}/editar` : `${baseTransportista}/crear`;
            ...
            try {
                ...
                if (json.ok) {
                    bootstrap.Modal.getInstance(document.getElementById('formTransportistaModal')).hide();
        ...
        // Editar/Eliminar transportista (delegación de eventos)
        document.querySelector('#tablaTransportistas tbody').addEventListener('click', async (e) => {
            const btn = e.target.closest('.btn-edit-transportista, .btn-del-transportista');
            if (!btn) return;

            const id = btn.dataset.id;

            if (btn.classList.contains('btn-edit-transportista')) {
                const res = await fetch(`${baseTransportista}/${id}/json`);
                const t = await res.json();
                document.getElementById('formTransportistaModalLabel').textContent = 'Editar Transportista';
                document.getElementById('t-id').value = t.id;
                document.getElementById('t-nombre').value = t.nombre || '';
                document.getElementById('t-contacto').value = t.contacto || '';
                document.getElementById('t-telefono').value = t.telefono || '';
                document.getElementById('t-email').value = t.email || '';
                new bootstrap.Modal(document.getElementById('formTransportistaModal')).show();
            } else if (btn.classList.contains('btn-del-transportista')) {
                Swal.fire({
                    title: "¿Estás seguro?",
                    ...
                    if (result.isConfirmed) {
                        const res = await fetch(`${baseTransportista}/${id}/eliminar`, {
                            method: 'DELETE',
                            headers: {'X-Requested-With': 'XMLHttpRequest'}
                        });
                        const json = await res.json();
        ...
        // Cargar transportistas al abrir el modal
        document.getElementById('transportistasModal').addEventListener('shown.bs.modal', cargarTransportistas);
        ...
        // === ELIMINAR ENVÍO CON SWEETALERT2 (EN ESPAÑOL) ===
        document.querySelectorAll('.form-eliminar-envio').forEach(form => {
